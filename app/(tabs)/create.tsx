...
export default function Create() {
    ...
    const store = async () =>{
        const arr = JSON.parse(await AsyncStorage.getItem('task')) || []
        arr.push({title: title, isDone : false})
        ...
        <View style={{ alignItems: 'center', paddingTop: 38, paddingLeft: 26, paddingRight: 26, width: '100%' }}>
            ...
            <View style={{ width: '100%' }}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 13, alignItems: 'center' }}>
                    <TextInput onChangeText={(text) => setTitle(text)} placeholder="Add a note..." style={{ fontFamily: 'SpaceMono', fontSize: 14, fontWeight: 500, color: '#C6CFDC' }}></TextInput>
                    <TouchableOpacity onPress={store} style={style.button}>+ Create task</TouchableOpacity>
                </View>
            </View>
        </View>
    )
...
const style = StyleSheet.create({
    ...
    text3: { fontFamily: 'UrbanistMedium', fontSize: 16, fontWeight: 500, color: '#8D9CB8', marginBottom: 32 },
